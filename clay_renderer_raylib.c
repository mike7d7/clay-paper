...
Ray GetScreenToWorldPointWithZDistance(Vector2 position, Camera camera, int screenWidth,
  ...
  if (camera.projection == CAMERA_PERSPECTIVE) {
    ...
    matProj = MatrixPerspective(camera.fovy * DEG2RAD, ((double)screenWidth / (double)screenHeight),
                                0.01f, zDistance);
...
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands, Font *fonts) {
  for (int j = 0; j < renderCommands.length; j++) {
    ...
    case CLAY_RENDER_COMMAND_TYPE_TEXT: {
      ...
      // Raylib uses standard C strings so isn't compatible with cheap slices, we need to clone the
      // string to append null terminator
    ...
    case CLAY_RENDER_COMMAND_TYPE_BORDER: {
      ...
      if (config->cornerRadius.topRight > 0) {
        DrawRing(
            (Vector2){roundf(boundingBox.x + boundingBox.width - config->cornerRadius.topRight),
